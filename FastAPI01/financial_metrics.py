...
def calculate_metrics(statements: dict) -> dict:
    ...
    inventory_turnover = safe_div(cost_of_rev, inventory)
    receivables_turnover = safe_div(rev, receivables)
    asset_turnover = safe_div(rev, total_assets)
    
    # Cash Conversion Cycle (rough estimate)
    # Days = 365 / Turnover
    dio = safe_div(365, inventory_turnover)
    dso = safe_div(365, receivables_turnover)
    
    payables = bal.get("accounts_payable")
    dpo = None
    if payables is not None and cost_of_rev:
...
def calculate_growth(statements: dict, historical: dict) -> dict:
    ...
    return {
        ...
        "revenue_growth_qoq": None, # Placeholder for now
        "net_income_growth_yoy": ni_growth,
        "earnings_growth": ni_growth, # Using NI as earnings proxy
