...
def extract_financial_data(facts_gaap: dict, text: str = "", year_offset: int = 0):
    # ── Income statement ────────────────────────────────────────────────────
    revenue = get_best_value(facts_gaap, REVENUE_TAGS, year_offset)
    if revenue is None:
        revenue = regex_parse_from_table(text, "Revenues")

    cost_of_revenue = get_best_value(facts_gaap, COST_TAGS, year_offset)
    ...
    if gross_profit is None:
        gross_profit = get_value(facts_gaap, "GrossProfit", year_offset)
    if gross_profit is None:
        gross_profit = regex_parse_from_table(text, "GrossProfit")

    operating_income = get_value(facts_gaap, "OperatingIncomeLoss", year_offset)
    if operating_income is None:
        operating_income = regex_parse_from_table(text, "OperatingIncomeLoss")

    net_income = get_value(facts_gaap, "NetIncomeLoss", year_offset)
    if net_income is None:
        net_income = regex_parse_from_table(text, "NetIncomeLoss")

    interest_expense = get_value(facts_gaap, "InterestExpense", year_offset)
    if interest_expense is None:
        interest_expense = regex_parse_from_table(text, "InterestExpense")

    income_tax = get_value(facts_gaap, "IncomeTaxExpenseBenefit", year_offset)
    ...
    # ── Balance sheet ────────────────────────────────────────────────────────
    total_assets = get_value(facts_gaap, "Assets", year_offset)
    if total_assets is None:
        total_assets = regex_parse_from_table(text, "Assets")

    total_liabilities = get_value(facts_gaap, "Liabilities", year_offset)
    if total_liabilities is None:
        total_liabilities = regex_parse_from_table(text, "Liabilities")

    shareholder_equity = get_value(facts_gaap, "StockholdersEquity", year_offset)
    if shareholder_equity is None:
        shareholder_equity = regex_parse_from_table(text, "StockholdersEquity")

    cash_and_equivalents = get_value(facts_gaap, "CashAndCashEquivalentsAtCarryingValue", year_offset)
    if cash_and_equivalents is None:
        cash_and_equivalents = regex_parse_from_table(text, "CashAndCashEquivalentsAtCarryingValue")

    short_term_debt = get_value(facts_gaap, "DebtCurrent", year_offset)
    if short_term_debt is None:
        short_term_debt = regex_parse_from_table(text, "DebtCurrent")

    long_term_debt = get_value(facts_gaap, "LongTermDebt", year_offset)
    if long_term_debt is None:
        long_term_debt = regex_parse_from_table(text, "LongTermDebt")

    current_assets = get_value(facts_gaap, "AssetsCurrent", year_offset)
    if current_assets is None:
        current_assets = regex_parse_from_table(text, "AssetsCurrent")

    current_liabilities = get_value(facts_gaap, "LiabilitiesCurrent", year_offset)
    if current_liabilities is None:
        current_liabilities = regex_parse_from_table(text, "LiabilitiesCurrent")

    receivables = get_value(facts_gaap, "AccountsReceivableNetCurrent", year_offset)
    if receivables is None:
        receivables = regex_parse_from_table(text, "AccountsReceivableNetCurrent")

    inventory = get_value(facts_gaap, "InventoryNet", year_offset)
    if inventory is None:
        inventory = get_value(facts_gaap, "InventoriesNet", year_offset)

    accounts_payable = get_value(facts_gaap, "AccountsPayableCurrent", year_offset)

    # ── Cash flow ────────────────────────────────────────────────────────────
    operating_cash_flow = get_value(facts_gaap, "NetCashProvidedByUsedInOperatingActivities", year_offset)
    if operating_cash_flow is None:
        operating_cash_flow = regex_parse_from_table(text, "NetCashProvidedByUsedInOperatingActivities")

    capital_expenditure = get_value(facts_gaap, "PaymentsToAcquirePropertyPlantAndEquipment", year_offset)
    if capital_expenditure is None:
        capital_expenditure = regex_parse_from_table(text, "PaymentsToAcquirePropertyPlantAndEquipment")

    depreciation = get_value(facts_gaap, "DepreciationDepletionAndAmortization", year_offset)
    if depreciation is None:
        depreciation = regex_parse_from_table(text, "DepreciationDepletionAndAmortization")

    dividends = get_value(facts_gaap, "PaymentsOfDividends", year_offset)
    if dividends is None:
        dividends = get_value(facts_gaap, "PaymentsOfDividendsCommonStock", year_offset)
...
def get_historical_data(facts_gaap: dict, year_offset: int = 0) -> dict:
    revenue_prev    = get_best_value(facts_gaap, REVENUE_TAGS, year_offset + 1)
    net_income_prev = get_value(facts_gaap, "NetIncomeLoss", year_offset + 1)
    ocf_prev        = get_value(facts_gaap, "NetCashProvidedByUsedInOperatingActivities", year_offset + 1)
    capex_prev      = get_value(facts_gaap, "PaymentsToAcquirePropertyPlantAndEquipment", year_offset + 1)
